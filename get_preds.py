...
if args.row in [3, 4]:
    pickle.dump([all_predictions, all_accuracies, each_predictions, agg_predictions, probabilities], open('results/row{0}_{1}.pkl'.format(args.row, 'min' if args.minority else 'maj'), 'wb'))
else:
    pickle.dump([all_predictions, all_accuracies, each_predictions, agg_predictions, probabilities], open('results/row{}.pkl'.format(args.row), 'wb'))
